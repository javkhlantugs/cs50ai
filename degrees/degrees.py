...
def shortest_path(source, target):
    ...
    #initialize frontier with just source star and explored set
    ...
    while True:
        ...
        if node.state == target:
            ...
            while node.parent is not None:
                starsList.append(node.state)
                moviesList.append(node.action)
                node = node.parent
            moviesList.reverse()
            starsList.reverse()
            return list(zip(moviesList, starsList))
